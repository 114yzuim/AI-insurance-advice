// 配置策略整頁 — 對應原型 AllocScreen / 截圖的「Step 04 配置策略」。
//
// 用法:
//   import { AllocationStrategy } from "./AllocationStrategy";
//   import "./styles.css";   // 可選的中性預設樣式
//   <AllocationStrategy client={client} baseUrl="/api/allocation" onNext={...} />
    ...
    (async () => {
      try {
        if (mode === "free") {
          ...
          if (ignore) return;
          setView({
            mode, allocation, needs, constraints: [], binding: [], feasible: true,
          });
        } else {
  ...
  return (
    <div className="alloc-strategy">
      <header className="alloc-strategy__head">
        <div className="alloc-strategy__eyebrow">Step 04 · Allocation Strategy</div>
        <h1 className="alloc-strategy__title">選擇配置策略</h1>
