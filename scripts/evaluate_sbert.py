"""
三模型評審 Sentence-BERT baseline (sbert_only)
讀取 sbert_results.json，用 Claude / GPT-4o / Gemini 評分，
輸出 sbert_eval_results.json。
"""
...
def eval_gpt4o(prompt):
    r = openai_client.chat.completions.create(
        model="gpt-4o", max_tokens=200,
        messages=[{"role": "system", "content": EVAL_SYSTEM}, {"role": "user", "content": prompt}],
...
def eval_gemini(prompt):
    r = gemini_client.models.generate_content(
        model="gemini-2.5-flash", contents=EVAL_SYSTEM + "\n\n" + prompt,
    )
...
def main():
    ...
    for item in items:
        ...
        row = {k: item[k] for k in ("scenario_id", "scenario_label", "mode", "run")}
        for ev_name, ev_fn in EVALUATORS.items():
            done += 1
            print(f"[{done}/{total}] {item['scenario_id']} sbert_only run{item['run']} | {ev_name}")
            scores = ev_fn(prompt)
            row[ev_name] = scores
            print(f"  D1={scores['D1']} D2={scores['D2']} D3={scores['D3']} W={scores['W_score']}")
            time.sleep(0.5)
        results.append(row)

    OUTPUT_PATH.write_text(json.dumps(results, ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"\n[OK] 完成！結果存至 {OUTPUT_PATH}")
    ...
    for row in results:
        for ev in EVALUATORS:
            ...
            if s["D1"] is None:
                continue
            sums[row["scenario_id"]]["D1"] += s["D1"]
            sums[row["scenario_id"]]["D2"] += s["D2"]
            sums[row["scenario_id"]]["D3"] += s["D3"]
            sums[row["scenario_id"]]["W"]  += s["W_score"]
            sums[row["scenario_id"]]["n"]  += 1

    print(f"{'情境':<8} {'D1':>5} {'D2':>5} {'D3':>5} {'W-Score':>8}")
    for sid in sorted(sums):
        v = sums[sid]
        n = v["n"]
        print(f"{sid:<8} {v['D1']/n:>5.2f} {v['D2']/n:>5.2f} {v['D3']/n:>5.2f} {v['W']/n:>8.3f}")

    # 整體均值
    all_d1 = all_d2 = all_d3 = all_w = 0
    n_total = 0
    for v in sums.values():
        all_d1 += v["D1"]; all_d2 += v["D2"]; all_d3 += v["D3"]
        all_w  += v["W"];  n_total += v["n"]
    print(f"{'Overall':<8} {all_d1/n_total:>5.2f} {all_d2/n_total:>5.2f} {all_d3/n_total:>5.2f} {all_w/n_total:>8.3f}")

    # Pearson r
    print("\n=== Pearson 相關係數（W-Score）===")
    ...
    for i in range(len(ev_list)):
        for j in range(i+1, len(ev_list)):
            ...
            print(f"  {a} x {b} : r = {pearson(w_by_ev[a], w_by_ev[b])}")
