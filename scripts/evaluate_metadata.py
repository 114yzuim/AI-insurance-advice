"""
metadata_rag 中間條件的三模型評審腳本（回應審查 P0）。
對 scripts/metadata_results.json 的 9 筆回應，分別用 Claude/GPT-4o/Gemini 評分（共 27 次）。
評分 prompt 與 rubric 與 evaluate_multimodel.py 完全相同，確保跨條件可比較。

執行：python scripts/evaluate_metadata.py
"""
...
RESULTS_PATH = Path(__file__).parent / "metadata_results.json"
OUTPUT_PATH  = Path(__file__).parent / "metadata_eval_results_v3.json"
